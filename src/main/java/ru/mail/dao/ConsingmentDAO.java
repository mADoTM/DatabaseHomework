...
public final class ConsingmentDAO implements DAO<Consingment> {
    ...
    public ConsingmentDAO(@NotNull Connection connection) {
    ...
    public void save(@NotNull Consingment entity) {
        try (var preparedStatement = connection.prepareStatement("INSERT INTO consingment (consingment_id, order_date, company_id) VALUES (?, ?, ?)")) {
    ...
    public void update(@NotNull Consingment entity) {
        try(var preparedStatement = connection.prepareStatement("UPDATE consingment SET order_date = ?, company_id = ? WHERE consingment_id = ?")) {
    ...
    public void delete(@NotNull Consingment entity) {
        try(var preparedStatement = connection.prepareStatement("DELETE FROM consingment WHERE consingment_id = ?")) {
            preparedStatement.setInt(1, entity.consingmentId());
